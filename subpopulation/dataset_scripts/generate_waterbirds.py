...
species = np.unique(
    [
        img_filename.split("/")[0].split(".")[1].lower()
...
for idx, target_places in enumerate(target_places):
    ...
    for target_place in target_places:
        ...
        print(
            f"train category {idx} {target_place_full} has id {target_place_ids[idx]}"
        )
...
for i in tqdm(df.index):
    ...
    seg_path = os.path.join(
        cub_dir, "segmentations", df.loc[i, "img_filename"].replace(".jpg", ".png")
    )
